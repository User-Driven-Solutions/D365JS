...
export default class FormClass extends ContextClass
{
  constructor(context) {
    ...
    filterOptions(controlName, filter) {
        ...
        const filteredOptions = Array.from(this.originalOptions.get(controlName)).filter(([value, text]) => filter(value, text) );
        ...
        if (filteredOptions.some(([value, text]) => value === currentValue)) {
