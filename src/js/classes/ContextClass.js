export default class ContextClass
{
    ...
    getSelectedLookup(controlName)
    {
        ...
        return control?._options.filter(option => selected.includes(option.value))
    }
    ...
    {
        this.formContextUI.setFormNotification(message, notificationType, messageName);
        ...
        propNames
            .forEach(propName => {
                ...
                componentFields.forEach(field => {
                    watches.has(field) ? watches.get(field).push(propName) : watches.set(field, [propName])
                })
            })
   ...
   */
    setRequired(targets, watch=true) {
        ...
        if(targets)
        {
            ...
            console.log(`Targets: ALL`);
        }

        targets
            .filter(propName => Object.hasOwn(this[propName], "required") || Object.hasOwn(this[propName], "visibleResult"))
            .forEach(propName => 
                {
                    let isRequired = false;
                    if (this[propName].visibleResult !== false && Object.hasOwn(this[propName], "required")){
   ...
   */    
    setVisible(targets, watch=true) {
        ...
        if(targets)
        {
            ...
            targets = Object.getOwnPropertyNames(this).filter(propName => typeof (this[propName]) != 'undefined' && Object.hasOwn(this[propName], "visible"))
            console.log(`Targets: ALL`);
        }
        ...
        targets            
            .forEach(propName => 
            {
                ...
                {
                    this.setSections({ visible: visible }, this[propName].tabName, propName);
                }
                else
                {
    ...
    _clearOnHide(targetName) {
        if(this[targetName].visibleResult !== false || this[targetName].clearOnHide !== true || this.getValue(targetName) == null)
        {
            return
        }
